...
Building.update = function(id, building, result){
 dbConn.query("UPDATE buildings SET title=?,author=?,adress=?,contacts=? WHERE id = ?", [building.title,building.author, building.adress, buildingßs.contacts, id], function (err, res) {
  if(err) {
   console.log("error: ", err);
    result(null, err);
}
