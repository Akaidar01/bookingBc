...
Client.update = function(id, Clients, result){
 dbConn.query("UPDATE clients SET FullName=?,DateFrom=?,DateTo=?,Money=? WHERE id = ?", [Clients.FullName, Clients.DateFrom, Clients.DateTo, Clients.Money, id], function (err, res) {
  if(err) {
   console.log("error: ", err);
    result(null, err);
}
